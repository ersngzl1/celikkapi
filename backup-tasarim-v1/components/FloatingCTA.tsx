    ...
    <div className="hidden lg:flex fixed bottom-5 right-5 z-[100] flex-col items-end gap-3">
      {showPhone && (
        <a href="tel:+903221234567"
          className="flex items-center gap-3 pl-4 pr-5 py-3 bg-red-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 animate-fade-up">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setShowPhone(!showPhone)}
          className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 ${
            showPhone ? "bg-slate-700 text-white" : "bg-red-600 text-white"
          }`}>
          {showPhone ? <X className="w-5 h-5" /> : <Phone className="w-5 h-5" />}
        </button>

        {/* WhatsApp - always visible, large, pulsing */}
