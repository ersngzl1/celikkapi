...
export default function LiveNotification() {
  ...
  useEffect(() => {
    // Ilk bildirim 8-15 saniye sonra
  ...
  useEffect(() => {
    if (dismissed) return;
    // Sonrakiler 20-40 saniye arayla
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-2xl border border-slate-200 p-3.5 flex items-start gap-3">
        <div className="w-9 h-9 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
          <CheckCircle2 className="w-4.5 h-4.5 text-green-600" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm text-slate-800 font-medium leading-snug">
            <strong>{notif.name}</strong> ({notif.city}) <span className="text-green-600">{notif.action}</span>
          </p>
