...
function KatalogContent() {
  ...
  return (
    <div className="min-h-screen pb-16">
      {/* Hero */}
      <div ref={heroRef} className="relative -mt-[1px] overflow-hidden" style={{ background: 'linear-gradient(135deg, #0A0A0A 0%, #161616 50%, #0A0A0A 100%)' }}>
        ...
        <div className="container-custom relative z-10" style={{ padding: '40px 24px 48px' }}>
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            <div className="max-w-xl">
              ...
              <h1 className="animate-slide-up-delay-1 font-serif text-4xl md:text-6xl font-extrabold text-white" style={{ lineHeight: '1.1' }}>
                Çelik Kapı<br />
            ...
            <div className="hidden md:grid grid-cols-2 gap-3 animate-slide-up-delay-2">
              {categoryStats.map((cat) => (
                <button
                  ...
                  style={{
                    padding: '14px 20px',
                    background: category === cat.value ? 'rgba(200, 165, 92, 0.12)' : undefined,
                    borderColor: category === cat.value ? 'rgba(200, 165, 92, 0.25)' : undefined,
                  }}
                >
      ...
      <div className="container-custom" style={{ marginTop: '20px' }}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-5 stagger-children">
              {filtered.map((door) => (
                <div key={door.id} className="animate-on-scroll group">
                  <div className="relative rounded-2xl overflow-hidden h-[320px] sm:h-[380px] md:h-[440px] card-hover-glow" style={{ border: '1px solid var(--border)' }}>
                    ...
                    <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10">
                      <span className="text-[9px] md:text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full font-bold shadow-lg" style={{ background: 'rgba(200, 165, 92, 0.9)', color: 'var(--bg-primary)' }}>{door.category}</span>
                      <div className="flex items-center gap-1.5 px-2 py-1 rounded-full" style={{ background: 'rgba(10, 10, 10, 0.6)', backdropFilter: 'blur(8px)', border: '1px solid var(--border)' }}>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 md:p-5 z-10">
                      ...
                      <h3 className="font-serif text-sm md:text-xl font-bold text-white mt-0.5 md:mt-1 mb-2.5 md:mb-4 transition-colors">{door.name}</h3>
                      ...
                      <div className="flex gap-1.5 md:gap-2 translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
                        ...
                        <Link href={`/urun/${door.id}`} className="flex-1 flex items-center justify-center gap-1 py-2.5 md:py-3 text-white text-[10px] md:text-xs font-bold rounded-xl transition-all" style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.15)' }}>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch gap-0 rounded-2xl overflow-hidden transition-all duration-400" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
                    <Link href={`/urun/${door.id}`} className="w-full sm:w-48 h-48 sm:h-auto flex-shrink-0 relative overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3">
                        <span className="text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full font-bold shadow-sm" style={{ background: 'rgba(200, 165, 92, 0.9)', color: 'var(--bg-primary)' }}>{door.category}</span>
                      </div>
                    </Link>
        ...
        {/* Bottom CTA */}
        <div className="animate-on-scroll-scale relative overflow-hidden" style={{ marginTop: '64px', borderRadius: '24px', background: 'linear-gradient(135deg, #0A0A0A 0%, #161616 50%, #0A0A0A 100%)', border: '1px solid var(--border)' }}>
          ...
          <div className="relative z-10 text-center" style={{ padding: '48px 24px' }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md mx-auto sm:max-w-none">
              ...
              <a href="tel:+903221234567" className="w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-3.5 text-white font-bold rounded-xl text-sm transition-all" style={{ border: '1px solid var(--border-light)' }}>
