...
export default function CelikKapiPage() {
  return (
    ...
    const loadData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Filter out room doors — only steel doors
          const steelDoors = data.filter((p: any) => p.category !== "oda" && p.category !== "Oda Kapısı");
          setDoorsList(steelDoors);
          // Derive categories
          const cats = [...new Set(steelDoors.map((d: any) => d.category))].filter(Boolean).sort() as string[];
          setCategories([
            { value: "all", label: "Tümü" },
            ...cats.map((c: string) => ({ value: c, label: c })),
          ]);
          // Derive colors
          const uniqueColors = [...new Set(steelDoors.map((d: any) => d.color))].filter(Boolean).sort() as string[];
          setColorOptions([
          ...
          <div className="animate-on-scroll-scale relative overflow-hidden" style={{ marginTop: '64px', borderRadius: '24px', background: 'var(--hero-gradient)', border: '1px solid var(--border)' }}>
            ...
            <div className="relative z-10 text-center" style={{ padding: '48px 24px' }}>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <a href={waLink} target="_blank" rel="noopener noreferrer" className="gtm-whatsapp flex items-center justify-center gap-2 px-7 py-3.5 bg-[#25D366] hover:bg-[#20BD5A] text-white font-bold rounded-xl text-sm transition-all wa-glow">
                ...
                <a href={`tel:${settings.phone.replace(/[^0-9+]/g, "")}`} className="gtm-phone flex items-center justify-center gap-2 px-7 py-3.5 text-white font-bold rounded-xl text-sm transition-all" style={{ border: '1px solid rgba(255,255,255,0.2)' }}>
