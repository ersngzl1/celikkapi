    ...
    <div className="hidden lg:flex fixed bottom-5 right-5 z-[100] flex-col items-end gap-3">
      {showPhone && (
        <a href={`tel:${settings.phone.replace(/[^0-9+]/g, "")}`}
          onClick={() => trackPhone("floating_cta")}
          className="gtm-phone flex items-center gap-3 pl-4 pr-5 py-3 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 animate-fade-up cta-gold"
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setShowPhone(!showPhone)}
          ...
          style={{
            background: showPhone ? 'var(--bg-elevated)' : 'linear-gradient(135deg, var(--gold), var(--gold-dark))',
            color: showPhone ? 'var(--text-primary)' : 'var(--bg-primary)',
            border: showPhone ? '1px solid var(--border)' : 'none',
          }}>
          {showPhone ? <X className="w-5 h-5" /> : <Phone className="w-5 h-5" />}
        </button>
        ...
        <a href={waLink}
          ...
          className="gtm-whatsapp flex items-center gap-2.5 pl-4 pr-5 py-3.5 bg-[#25D366] text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 wa-glow wa-pulse">
